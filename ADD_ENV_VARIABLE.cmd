...
:: Use \ as separator
:: Must not end with \
set SDKPATH="C:\Dev\PlaydateSDK"

if %SDKPATH% == "YOUR CUSTOM SDK PATH HERE" (
	for /F "tokens=2* skip=2" %%a in ('reg query "HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders" /v "Personal"') do (
    set SDKPATH=%%b
  )
)
